...
```{r, echo=FALSE, warning=FALSE, message=FALSE, include=TRUE}
ms_new <- data.frame(t(newPreds$score_table[,c("AMR", "aTCMR", "caTCMR", "ATI", "IFTA")]))
## exclude 'normal' molecular score

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "AMR", "aTCMR", "caTCMR", "ATI", "normal", "IFTA", "Q")]
...
## reference biopsy chronic active TCMR molecular score ranges
...
```
Diagnosis based scores represent the probability that a biopsy has an expression profile similar to reference samples with a given histology based diagnosis: AMR, acute TCMR, chronic active TCMR, ATI (Acute tubular injury without rejection), atrophy/fibrosis (isolated IFTA).
