...
ms_new <- data.frame(t(newPreds$score_table[,c("AMR", "TCMR", "IFTA", "OTHER")]))

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "AMR", "TCMR", "OTHER", "IFTA", "Q")]
...
## reference biopsy OTHER molecular score ranges
tab[rownames(tab)=="OTHER","No lesion Q1"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="25%","OTHER"]
tab[rownames(tab)=="OTHER","No lesion Q3"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="75%","OTHER"]
...
ms_new <- data.frame(t(newPreds$score_table[,c("cg0_binary", "iifta0_binary", "v0_binary", "cv1_binary", "ci1_binary", "ct1_binary")]))
...
bkv_df$BKVP1 <- as.numeric(bkv_df$BKVP1)
...
new_normal_results <- ifelse(bkv_df[1,c("BKlargeTAg", "BKVP1")] > 0.05, "BKV expression is not significantly higher than normal biopsies",
			     ifelse(bkv_df[1,c("BKlargeTAg", "BKVP1")] > 0.01 & bkv_df[1,c("BKlargeTAg", "BKVP1")] <= 0.05, "BKV expression is moderately higher than normal biopsies",
			     "BKV expression is significantly higher than normal biopsies"))
# alternate hypothesis=new < BKV
new_bkv_results <- ifelse(bkv_df[1,c("BKlargeTAg", "BKVP1")] > 0.05, "BKV expression is not signficiantly lower than BKV biopsies",
			     ifelse(bkv_df[1,c("BKlargeTAg", "BKVP1")] > 0.01 & bkv_df[1,c("BKlargeTAg", "BKVP1")] <= 0.05, "BKV expression is moderately lower than BKV biopsies",
