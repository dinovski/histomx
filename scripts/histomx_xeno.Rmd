...
rmarkdown::render('../scripts/histomx_xeno.Rmd')
## convert output html to paged PDF 
system('wkhtmltopdf --javascript-delay 1 ~/Desktop/histomx_xeno.html ~/Desktop/histomx_xeno.pdf')
```
...
newPreds <- XenoPred(newRCC, outPath, saveFiles=FALSE)
dev.off()
...
ms_new <- data.frame(t(newPreds$score_table[,c("a_amr", "ca_amr", "a_tcmr", "ca_tcmr", "ifta")]))
## exclude 'normal' molecular score

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "a_amr", "ca_amr", "a_tcmr", "ca_tcmr", "normal", "ifta", "Q")]
...
## reference biopsy aAMR molecular score ranges
tab[rownames(tab)=="a_amr","No lesion Q1"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="25%","a_amr"]
tab[rownames(tab)=="a_amr","No lesion Q3"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="75%","a_amr"]

## reference biopsy caAMR molecular score ranges
tab[rownames(tab)=="ca_amr","No lesion Q1"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="25%","ca_amr"]
tab[rownames(tab)=="ca_amr","No lesion Q3"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="75%","ca_amr"]

## reference biopsy acute TCMR molecular score ranges
tab[rownames(tab)=="a_tcmr","No lesion Q1"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="25%","a_tcmr"]
tab[rownames(tab)=="a_tcmr","No lesion Q3"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="75%","a_tcmr"]

## reference biopsy chronic actve TCMR molecular score ranges
tab[rownames(tab)=="ca_tcmr","No lesion Q1"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="25%","ca_tcmr"]
tab[rownames(tab)=="ca_tcmr","No lesion Q3"] <- ms_ref[ms_ref$Dx=="No specific Dx" & ms_ref$Q=="75%","ca_tcmr"]
...
dx_tab[dx_tab$Diagnosis=="a_amr",1]<-"aAMR"
dx_tab[dx_tab$Diagnosis=="ca_amr",1]<-"caAMR"
dx_tab[dx_tab$Diagnosis=="a_tcmr",1]<-"aTCMR"
dx_tab[dx_tab$Diagnosis=="ca_tcmr",1]<-"caTCMR"
...
```
Diagnosis based scores represent the probability that a biopsy has an expression profile similar to reference samples with a given histology based diagnosis: AMR, TCMR, and atrophy/fibrosis (isolated IFTA).
...
banff_preds <- data.frame(t(newPreds$score_table))
...
kable(banff_tab, row.names=FALSE, align='c', format="html") %>%
    column_spec(1, bold=TRUE, color="black") %>%
...
ms_new <- data.frame(t(newPreds$score_table[,c("cg0_score", "iifta0_score", "v0_score", "cv1_score", "ci1_score", "ct1_score")]))

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "cg0_score", "iifta0_score", "v0_score", "cv1_score", "ci1_score", "ct1_score", "Q")]
 
## q1 and q3 (can also add IQR, min, max)
tab <- data.frame(matrix(nrow=dim(ms_new)[1], ncol=length(unique(ms_ref$Dx))*2))
colnames(tab) <- c("aAMR Q1", "aAMR Q3", "caAMR Q1", "caAMR Q3",
		   "aTCMR Q1", "aTCMR Q3", "caTCMR Q1", "caTCMR Q3",
                   "IFTA Q1", "IFTA Q3", "No lesion Q1", "No lesion Q3")
rownames(tab) <- rownames(ms_new)

## add reference biopsy molecular score ranges
## rows=molecular scores; columns=histo diagnosis
for(i in 1:nrow(tab)) {
  tab[rownames(tab)==rownames(ms_new)[i],"aAMR Q1"] <- ms_ref[ms_ref$Dx=="aAMR" & ms_ref$Q=="25%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"aAMR Q3"] <- ms_ref[ms_ref$Dx=="aAMR" & ms_ref$Q=="75%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"caAMR Q1"] <- ms_ref[ms_ref$Dx=="caAMR" & ms_ref$Q=="25%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"caAMR Q3"] <- ms_ref[ms_ref$Dx=="caAMR" & ms_ref$Q=="75%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"aTCMR Q1"] <- ms_ref[ms_ref$Dx=="aTCMR" & ms_ref$Q=="25%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"aTCMR Q3"] <- ms_ref[ms_ref$Dx=="aTCMR" & ms_ref$Q=="75%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"caTCMR Q1"] <- ms_ref[ms_ref$Dx=="caTCMR" & ms_ref$Q=="25%",rownames(ms_new)[i]]
  tab[rownames(tab)==rownames(ms_new)[i],"caTCMR Q3"] <- ms_ref[ms_ref$Dx=="caTCMR" & ms_ref$Q=="75%",rownames(ms_new)[i]]
...
#tab <- tab[!is.na(colnames(tab))]
tab$'aAMR range' <- paste(tab$`aAMR Q1`, "-", tab$`aAMR Q3`)
tab$'caAMR range' <- paste(tab$`caAMR Q1`, "-", tab$`caAMR Q3`)
tab$'aTCMR range' <- paste(tab$`aTCMR Q1`, "-", tab$`aTCMR Q3`)
tab$'caTCMR range' <- paste(tab$`caTCMR Q1`, "-", tab$`caTCMR Q3`)
tab$'IFTA range' <- paste(tab$`IFTA Q1`, "-", tab$`IFTA Q3`)
tab$'Normal range' <- paste(tab$`No lesion Q1`, "-", tab$`No lesion Q3`)
tab$'lesion' <- gsub("_", " ", rownames(tab))
...
ref_quantiles$'lesion' <- gsub("0 score", " > 0", ref_quantiles$'lesion')
ref_quantiles$'lesion' <- gsub("1 score", " > 1", ref_quantiles$'lesion')
...
***
### Pathway analysis
```{r, echo=FALSE, warning=FALSE, comment=FALSE, results='asis', fig.align='center', fig.cap='Genes with increased expression in the tested biopsy compared to all normal biopsies associated with the listed pathway (numbered dots indicate the # of enriched genes; % = genes associated with the pathway).'}
