...
ms_new <- data.frame(t(newPreds$score_table[,c("AMR", "TCMR", "IFTA")]))
## exclude 'normal' molecular score

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "AMR", "TCMR", "normal", "IFTA", "Q")]
...
```
The above Banff lesion based scores represent the probability for each lesion.
...
```
Banff lesion based scores represent the probability that a biopsy has a Banff score >0 or >1, as indicated for each lesion: unlikely (<10%), low (10-20%), moderate (20-40%), moderate/high (40-60%), high (60-80%), very high (80-100%).
...
```{r, echo=FALSE, warning=FALSE, comment=FALSE, results='asis', fig.align='center',  fig.width=4, fig.height=3, fig.cap='Normalized expression of Polyomavirus BK VP1 and Polyomavirus BK Large T Antigen genes in the test sample and reference biopsies with BKV or no specific diagnosis/lesions'}
...
bkv_df <- newPreds$bkv_stats
bkv_df <- data.frame(apply(bkv_df, 1, as.character))
rownames(bkv_df) <- colnames(newPreds$bkv_stats)
bkv_df <- bkv_df[rownames(bkv_df) %in% c("new_normal_pval", "new_bkv_pval"),]

## interpretation (if both pvalues sig)
# alternate hypothesis=higher
new_normal_results <- ifelse(bkv_df[1,] > 0.05, "BKV expression is not significantly higher than normal biopsies",
			     ifelse(bkv_df[1,] < 0.01, "BKV expression is moderately higher than normal biopsies",
			     "BKV expression is significantly higher than normal biopsies"))
# alternate hypothesis=lower
new_bkv_results <- ifelse(bkv_df[1,] >= 0.05, "BKV expression is not signficiantly high compared to BKV biopsies",
			     ifelse(bkv_df[1,] < 0.01, "BKV expression is moderately similar to BKV biopsies",
			     "BKV expression is signficiantly similar to normal biopsies"))

## rename results
#rownames(bkv_df)[rownames(bkv_df)=="normal_median"]<-"normal samples median expression"
#rownames(bkv_df)[rownames(bkv_df)=="bkv_median"]<-"BKV samples median expression"
#rownames(bkv_df)[rownames(bkv_df)=="new_counts"]<-paste(newID, "mean expression")
#rownames(bkv_df)[rownames(bkv_df)=="bkv_normal_pval"]<-"BKV v. normal biopsies p-value"
rownames(bkv_df)[rownames(bkv_df)=="new_normal_pval"]<-paste(newID, "v. normal biopsies p-value")
rownames(bkv_df)[rownames(bkv_df)=="new_bkv_pval"]<-paste(newID, "v. BKV biopsies p-value")

bkv_df$Interpretation <- c(new_normal_results[1], new_bkv_results[1])

kable(bkv_df, row.names=TRUE, align=rep('c', 3), format="html") %>%
           row_spec(1:nrow(bkv_df), background="white") %>% column_spec(c(1), bold=TRUE, background="whitesmoke")
	
```
p-values were computed using a Wilcoxon rank-sum test to compare BKV gene expression.
