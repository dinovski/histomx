...
system('wkhtmltopdf --javascript-delay 1 histomx_report.html histomx_report.pdf')
```
...
> **Team leader:** Pr Alexandre Loupy
**Project manager:** Blaise Robin  
...
if(!is.na(file.info(newPatient)$size)) {
    ...
    pt_df <- data.frame('Platform ID'=newPreds$score_table$ID,
                        ...
                        'Donor type'='\\t+', 'Donor age'="    ", 'Biopsy indication'="    ", check.names=FALSE)
...
```
All dates are reported as mm/dd/yyyy  
...
ms_new <- data.frame(t(newPreds$score_table[,c("g0_score", "ptc0_score", "cg0_score", "i1_score", "t1_score", "v0_score", "cv1_score", "ci1_score", "ct1_score")]))

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "g0_score", "ptc0_score", "cg0_score", "i1_score", "t1_score", "v0_score", "cv1_score", "ci1_score", "ct1_score", "Q")]
...
### PCA labeled by histologic diagnosis
<div style="margin-bottom:20px;">
```{r, echo=FALSE, warning=FALSE, comment=FALSE, results='asis', fig.align='center', fig.cap='The tested sample is represented by the yellow diamond.'}
newPreds$pca_1_2
```
</div>

<div style="margin-bottom:20px;">
```{r, echo=FALSE, warning=FALSE, comment=FALSE, results='asis', fig.align='center'}
newPreds$pca_2_3
```
</div>
...
### Pathway analysis
<div style="margin-bottom:20px;">
```{r, echo=FALSE, warning=FALSE, comment=FALSE, results='asis', fig.align='center', fig.cap='Enrichment of genes associated with the listed pathway with expression in the tested biopsy greater than 2 standard deviations of the median expression in all normal biopsies.'}
...
### Cell-type analysis
<div style="margin-bottom:20px;">
```{r, echo=FALSE, warning=FALSE, comment=FALSE, results='asis', fig.align='center', fig.cap='Enrichment of genes associated with the listed cell type with expression in the tested biopsy greater than 2 standard deviations of the median expression in all normal biopsies.'}
...
kable(data.frame(Name=">", Signature=""), align='c', format="html") %>% kableExtra::kable_styling(position="center") %>%
...
if(!is.na(file.info(newRNA)$size)) {
  ...
  ## if units (ng/ul) included in RNA concentration strip
