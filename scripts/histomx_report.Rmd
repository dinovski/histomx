...
tab$'Molecular score' <- rownames(tab)

#dx_tab <- tab[tab$'Molecular score' != "No specific Dx",] ## exclude 'normal' molecular score
dx_tab <- cbind(ms_new, tab[,c("Molecular score", "Normal range")])
dx_tab <- dx_tab %>% relocate('Molecular score', .before = colnames(dx_tab)[1])
...
ms_new <- data.frame(t(newPreds$score_table[,c("g0_score", "ptc0_score", "cg0_score", "i1_score", "t1_score", "v0_score", "cv0_score", "ci0_score", "ct0_score")]))

## reference set quantiles
ms_ref <- newPreds$ref_scores[,c("Dx", "g0_score", "ptc0_score", "cg0_score", "i1_score", "t1_score", "v0_score", "cv0_score", "ci0_score", "ct0_score", "Q")]
...
tab$'Normal range' <- paste(tab$`No lesion Q1`, "-", tab$`No lesion Q3`)
tab$'Molecular score' <- gsub("_", " ", rownames(tab))
...
ref_quantiles <- tab[,c(newID, "Molecular score", "Normal range", "Interpretation")]

ref_quantiles$'Molecular score' <- gsub("0 score", " > 0", ref_quantiles$'Molecular score')
ref_quantiles$'Molecular score' <- gsub("1 score", " > 1", ref_quantiles$'Molecular score')

ref_quantiles <- ref_quantiles %>% relocate('Molecular score', .before = colnames(ref_quantiles)[1])
